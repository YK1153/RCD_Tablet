...
namespace CommWrapper
{
    public abstract class CommMsgBase
    {
        ...
        {
            PropertyInfo[] props = typeof(CommMsgBase).GetProperties();
            PropertyInfo msgIdProp = typeof(CommMsgBase).GetProperty("MSGID");
    ...
    public class RcvResultSendMsg : CommMsgBase, ISndMsgBase
    {
        ...
        public static RcvResultSendMsg GetOK()
        {
            RcvResultSendMsg msg = new RcvResultSendMsg();
            msg.result = 00000;

            return msg;
        }

        public static RcvResultSendMsg GetOK(int serialNo)
        {
            RcvResultSendMsg msg = new RcvResultSendMsg();
        ...
        public static RcvResultSendMsg GetModeNotAvailableMsg(int serialNo)
        {
            RcvResultSendMsg msg = new RcvResultSendMsg();
            msg.SerialNo = serialNo;
            msg.result = 11111;
        ...
        public RcvResultSendMsg()
        {
            MSGID = msgId;
        }
    }

    public abstract class CommSndMsgBase : CommMsgBase, ISndMsgBase
    {
        // 送信メッセージID別シリアル採番リスト
        ...
        protected CommSndMsgBase(string msg_id)
        {
            MSGID = msg_id;
            lock (dictlock)
            {
    ...
    public class RcvRsultRcvMsg : CommMsgBase
    {
        public const string msgId = "00";
        ...
        public RcvRsultRcvMsg(string rcv_msg)
        {
    ...
    public class RcvMsgBase : CommMsgBase
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        /// <param name="rcv_msg"></param>
        public RcvMsgBase(string rcv_msg)
        {
            List<PropertyInfo> orderedProps = new List<PropertyInfo>(GetType().GetProperties())
                .OrderBy(prop => OrderAttribute.GetOrder(prop))
                .ToList();
