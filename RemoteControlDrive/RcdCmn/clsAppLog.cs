...
namespace RcdCmn
{
    public class AppLog : IDisposable
    {
        ...
        // ADD 20230524
        private static object LOCK_QUE = new object();
        private static Thread m_LogWriterThread;
        ...
        public AppLog()
        {
            try
            {
                ...
                // ADD 20230524
                m_cancel = false;
                m_LogWriterThread = new Thread(new ThreadStart(LogWriter));
        ...
        private void WriteLog(string log, string debug_level, string method_name, int line)
        {
            try
            {
                ...
                lock (LOCK_QUE)
                {
                    m_queue.Enqueue($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")},[{System.Threading.Thread.CurrentThread.ManagedThreadId}],[{debug_level}],{method_name},({line}),-,{log}");
                }
                //
            }
            catch (Exception) { }
        ...
        private void LogWriter()
        {
            do
            {
                if (m_queue.Count > 0)
                {
                    ...
                    lock (LOCK_QUE)
                    {
                        log = m_queue.Dequeue().ToString();
                    }
                    lock (LOCK_OBJ)
                    {
                        GetWriter().WriteLine(log);
                        GetWriter().Flush();
                    }
                }
