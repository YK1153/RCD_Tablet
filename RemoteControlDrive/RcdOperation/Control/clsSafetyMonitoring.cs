...
using static RcdDao.MImageComparisolDao;
...
namespace RcdOperation.Control
{
    ...
    public partial class ControlMain
    {
        ...
        //※
        private const string C_NM_SAFETYMONITORING_INTRUSION = "[SaMoIn]";
        private const string C_NM_SAFETYMONITORING_COMPARISON = "[SaMoCo]";
        private readonly bool C_ALRET_MODE = bool.Parse(ConfigurationManager.AppSettings["C_ALRET_MODE"]);
        private readonly bool C_ALRET_MODE_COMPARISON = bool.Parse(ConfigurationManager.AppSettings["C_ALRET_MODE_COMPARISON"]);
        //
        ...
        private List<ObjectLabel> m_labelList;
        ...
        //# ローカライズ比較 # 
        List<MImageComparisol> m_deviations;
        /// <summary>
        /// ローカライズ不一致カウント
        /// </summary>
        private int m_cnt;
        ...
        {
            // 侵入検知
            m_pause_detect_flg =　OperationConst.C_NORMAL;
            m_controlCenterData.deceleration_rate = 1.0;
        ...
        private void Callback_DetectJudge(object state)
        {
            ...
            try
            {
                ...
                LOGGER.Debug($"{C_NM_SAFETYMONITORING_INTRUSION}確認範囲,slow:{m_slowfrontidx}=>{m_slowbackidx},pause:{m_pausefrontidx}=>{m_pausebackidx},stop:{m_stopfrontidx}=>{m_stopbackidx}");
                ...
                if (m_calcstarted == OperationConst.C_RECEIVED && m_detectCl.RcvStatus)
                {
                    ...
                    // 計算後車両位置を取得し、左後端へ変換
                    PointF controlcarpoint = GetCamLeftBackPoint(MainC);
                    LOGGER.Debug($"{C_NM_SAFETYMONITORING_INTRUSION}制御車両左後端：{controlcarpoint.X},{controlcarpoint.Y}");
                    ...
                    foreach (InstrusionDetection objects in objectDatas)
                    {
                        ...
                        if (m_carLabel == objects.objectData.label)
                        {
                            ...
                            LOGGER.Debug($"{C_NM_SAFETYMONITORING_INTRUSION}距離:{dist}：{objects.Position.X},{objects.Position.Y}");

                            if (dist <= C_CONTROLCARRADIUS)
                            {
                                objects.JudgesTarget = false;
                                LOGGER.Debug($"{C_NM_SAFETYMONITORING_INTRUSION}制御車両:{objects.Position.X},{objects.Position.Y}");
                            }
                        }
                    ...
                    foreach (InstrusionDetection objects in objectDatas)
                    {
                        ...
                        for (int i = m_slowbackidx; i < m_slowfrontidx; i++)
                        {
                            ...
                            if (slow)
                            {
                                string msg = $"{C_NM_SAFETYMONITORING_INTRUSION}減速エリア侵入:{objects.objectData.label}：{objects.Position.X},{objects.Position.Y}：{objects.Radius}：{Viewer.m_SpeedAreas.ElementAt(i).Value.ID}";
                                msg += $"：slowarea:";
                    ...
                    if (slow_contain)
                    {
                        ...
                        foreach (InstrusionDetection objects in objectDatas)
                        {
                            ...
                            for (int i = m_pausebackidx; i < m_pausefrontidx; i++)
                            {
                                ...
                                if (pause)
                                {
                                    string msg = $"{C_NM_SAFETYMONITORING_INTRUSION}一時停止エリア侵入:{objects.objectData.label}：{objects.Position.X},{objects.Position.Y}：{objects.Radius}：{Viewer.m_SpeedAreas.ElementAt(i).Value.ID}";
                                    msg += $"：pausearea:";
                    ...
                    if (pause_contain)
                    {
                        ...
                        foreach (InstrusionDetection objects in objectDatas)
                        {
                            ...
                            for (int i = m_stopbackidx; i < m_stopfrontidx; i++)
                            {
                                ...
                                if (stop)
                                {
                                    string msg = $"{C_NM_SAFETYMONITORING_INTRUSION}走行停止エリア侵入:{objects.objectData.label}：{objects.Position.X},{objects.Position.Y}：{objects.Radius}：{Viewer.m_SpeedAreas.ElementAt(i).Value.ID}";
                                    msg += $"：stoparea:";
                    ...
                    if (slow_contain || pause_contain || stop_contain)
                    {
                        ...
                        if (stop_contain)
                        {
                            ...
                            {
                                LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}走行停止エリア侵入→車両進行中のため走行停止");
                                // 走行停止
                                if (C_ALRET_MODE) Task.Run(() => EmergencyEndProcess($"{MethodBase.GetCurrentMethod().Name}", Res.ErrorStatus.DETECT_AREA_ERR.Code));
                            }
                            // 車両が止まっていたら一時停止
                            else
                            {
                                LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}走行停止エリア侵入→車両進行中のため一時停止");
                                // 一時停止
                        ...
                        {
                            LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}一時停止エリア侵入→一時停止");
                            // 一時停止
                            if (C_ALRET_MODE) m_pause_detect_flg = OperationConst.C_ABNORMAL;
                        }
                        ...
                        {
                            LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}減速エリア侵入→減速");
                            // 目標速度の変更
                            ...
                            if (m_pause_detect_flg ==　OperationConst.C_ABNORMAL)
                            {
                                ...
                                LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}Resuming running because the recognized object is out of the area");
                            }
                        }
                    }
                    // どこにも含まれていない場合
                    else
                    {
                        ...
                        {
                            LOGGER.Info($"{C_NM_SAFETYMONITORING_INTRUSION}減速エリア侵入解除→減速終了");
            ...
            {
                Task.Run(() => EmergencyEndProcess($"{MethodBase.GetCurrentMethod().Name}", Res.ErrorStatus.DETECT_AREA_ERR.Code));
        ...
        private void Callback_ComparisonPosition(object state)
        {
            ...
            {
                // タイマーストップ
                m_timer_comparison.Change(Timeout.Infinite, Timeout.Infinite);
                ...
                {
                    // 制御指示計算実行後実施
                    if (m_calcstarted == OperationConst.C_RECEIVED)
                    {
                        // 主測位中の測位情報を取得
                        //CamInfo maininfo = m_camClMain.CamRcvInfo;
                        CamInfo maininfo = GetCamRcvInfo(MainC);
                        PointF mainpoint = GetCamLeftBackPoint(MainC);
                        ...
                        for (int i = 0; i < subCamHist.Count(); i++)
                        {
                            ...
                            if (i > 0)
                            {
                                ...
                                {
                                    // 差が小さくなったので最新として取得し、次のデータと比較のため次へ
                                    // 主と副のデータの時間の差分を取得
                            ...
                            else
                            {
                                ...
                                // 主と副のデータの時間の差分を取得
                                span = maindatetime - infodatetime;
                                subinfo = subCamHist[i];
                            }
                        }

                        // サブ測位の座標セット
                        PointF subpoint = new PointF();
                        if (SubCamType == CamType.AVP)
                        {
                            PointF rearCenter = new PointF((float)(subinfo.xPosition), (float)(subinfo.yPosition));
                            double carYawrate = CommonProc.ToRadian(subinfo.MoveVector);
                            ...
                            subpoint.X = (float)(rearCenter.X - LLCY - TSY);
                            subpoint.Y = (float)(rearCenter.Y - LLSY + TCY);
                        }
                        else
                        {
                            subpoint.X = subinfo.xPosition;
                            subpoint.Y = subinfo.yPosition;
                        }

                        LOGGER.Info($"{C_NM_SAFETYMONITORING_COMPARISON}比較情報:主{MainCamType}{{{maininfo.camStartTime},({maininfo.xPosition},{maininfo.yPosition})}},{maininfo.MoveVector},副{SubCamType}{{{subinfo.camStartTime},({subinfo.xPosition},{subinfo.yPosition})}},{subinfo.MoveVector}");
                        LOGGER.Info($"{C_NM_SAFETYMONITORING_COMPARISON}比較情報:主{MainCamType}{{{maininfo.camStartTime},({mainpoint.X},{mainpoint.Y})}},{maininfo.MoveVector},副{SubCamType}{{{subinfo.camStartTime},({subpoint.X},{subpoint.Y})}},{subinfo.MoveVector}");

                        // 偏差の楕円を作成
                        ELLIPSE mainellipse = new ELLIPSE();
                        mainellipse.fRad_X = m_deviations.Single(val => (OperationConst.CamType)val.CamType == MainCamType).DeviationX;
                        mainellipse.fRad_Y = m_deviations.Single(val => (OperationConst.CamType)val.CamType == MainCamType).DeviationY;
                        mainellipse.fCx = mainpoint.X;
                        mainellipse.fCy = mainpoint.Y;

                        ELLIPSE subellipse = new ELLIPSE();
                        subellipse.fRad_X = m_deviations.Single(val => (OperationConst.CamType)val.CamType == SubCamType).DeviationX;
                        subellipse.fRad_Y = m_deviations.Single(val => (OperationConst.CamType)val.CamType == SubCamType).DeviationY;
                        subellipse.fCx = subpoint.X;
                        subellipse.fCy = subpoint.Y;

                        // 楕円の衝突判定
                        double dist = Math.Sqrt(Math.Pow(mainpoint.X - subpoint.X, 2) + Math.Pow(mainpoint.Y - subpoint.Y, 2));
                        LOGGER.Debug($"{C_NM_SAFETYMONITORING_COMPARISON}距離:{dist}");
                        ...
                        if (!CollisionEllipse(mainellipse, subellipse))
                        {
                            ...
                            {
                                Task.Run(() => EmergencyEndProcess($"{MethodBase.GetCurrentMethod().Name}",Res.ErrorStatus.COMPARISON_POSITION_ERROR.Code));
                            }
                            else
                            {
                                m_cnt++;
                        ...
                        // ベクトルの比較
                        double diff = maininfo.MoveVector - subinfo.MoveVector;
                        double mainVector = m_deviations.Single(val => (OperationConst.CamType)val.CamType == MainCamType).DeviationVecter;
                        double subVector = m_deviations.Single(val => (OperationConst.CamType)val.CamType == SubCamType).DeviationVecter;
                        LOGGER.Debug($"{C_NM_SAFETYMONITORING_COMPARISON}差:{diff}");
                        if (Math.Abs(diff) > mainVector + subVector)
                        {
                            if (m_cnt >= C_COMPARISONPOSITION)
                            {
                                LOGGER.Info("vector difference too large");
                                Task.Run(() => EmergencyEndProcess($"{MethodBase.GetCurrentMethod().Name}", Res.ErrorStatus.COMPARISON_POSITION_ERROR.Code));
                            }
                            else
                            {
            ...
            {
                //タイマー再開
                if (m_error_flg == OperationConst.C_NORMAL && m_timer_detect != null) m_timer_comparison.Change(OperationConst.C_SYSTEM_INTERVAL, OperationConst.C_SYSTEM_INTERVAL);
                AppLog.GetInstance().Debug($"{MethodBase.GetCurrentMethod().Name} End");
            }
        ...
        private bool CollisionEllipse(ELLIPSE E1, ELLIPSE E2)
        {
            ...
            LOGGER.Debug($"{C_NM_SAFETYMONITORING_COMPARISON}比較値：{Math.Floor(JudgeValue * 100000) / 100000}");
            if (JudgeValue <= 1)
                return true;    // 衝突
